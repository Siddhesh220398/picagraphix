...
function PichagraphixPhoto() {
  const userLogin = useSelector((state) => state.userLogin);
  const { error, loading, userInfo } = userLogin;
  const [photo, getPhoto] = useState([]);
  const [page, setPage] = useState(1);
  const [pageUrl, setPageUrl] = useState(`${Urls.base_url}photos/latestfilter`);
  ...
  useEffect(async () => {
    // await getpichaphoto(pageUrl);
    await getpichacategories();
    await getData(pageUrl);
  }, [page, productSchedule, productCategory]);
  // const getpichaphoto = async (pageUrl) => {
  //   await axios
  //     .get(pageUrl)
  //     .then(async (response) => {
  //       console.log(response)
  //       await getPhoto(response.data.data);
  //     })
  ...
  //   getData(pageUrl,productCategory,e)
  // };
...
export default PichagraphixPhoto;
