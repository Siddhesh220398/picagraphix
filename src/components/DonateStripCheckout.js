...
const CheckoutForm = (props) => {
    ...
    stripe.retrievePaymentIntent(clientSecret)
      .then(({ paymentIntent }) => {
        ...
        switch (paymentIntent.status) {
          case "succeeded":
            Notification('success',"Payment succeeded!");
            break;
          case "processing":
  ...
  const handleSubmit = async (e) => {
    ...
    const { error } = await stripe.confirmPayment({
      elements,
      confirmParams: {
        return_url: `http://localhost:3000/DonateSuccess/${props.orderId}`,
      },
    });
...
export const DonateStripCheckout = (props) => {
  ...
  useEffect(() => {
    ...
    axios.post(Urls.base_url + "create-donate", formData)
      .then(res => {
...
export default DonateStripCheckout